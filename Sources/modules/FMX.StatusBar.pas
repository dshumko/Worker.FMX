...
{$IF defined(ANDROID)}
    , Androidapi.JNIBridge, Androidapi.Helpers, Androidapi.JNI.OS, Androidapi.JNI.JavaTypes, Androidapi.Jni,
      Androidapi.JNI.App
{$ELSEIF defined(IOS)}
...
type
  TmyWindow = record
    ...
    class var NavigationBarHeight: Single;
...
begin
  ...
  TmyWindow.NavigationBarHeight := 0;
  {$IFDEF ANDROID}
  ...
  begin
    ...
    resourceID := TAndroidHelper.Activity.getResources.getIdentifier(StringToJString('navigation_bar_height'),
      StringToJString('dimen'), StringToJString('android'));
    if resourceID > 0 then
      TmyWindow.NavigationBarHeight := Trunc(TAndroidHelper.Activity.getResources.getDimensionPixelSize(resourceID)
                                             / sScale);
  end;
  {$ENDIF}
  Result := True;
